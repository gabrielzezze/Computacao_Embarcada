...
void handle_led_status(char led_status) {
	if(strcmp(led_status, '1')) pio_set(LED1_PIO, LED1_IDX_MASK);
	else if(strcmp(led_status, '0')) pio_clear(LED1_PIO, LED1_IDX_MASK);
}
  ...
  while(1){
    ...
    switch(state){
      ...
      if(xQueueReceive(xQueueMsg, &p_recvMsg, 5000) == pdTRUE){
        ...
        printf(STRING_EOL);  printf(STRING_LINE);
		char led[] = "led";
		char *led_key = strstr(p_recvMsg->pu8Buffer, led);
		handle_led_status(led_key[7]);
        state = DONE;
      }
      else {
        state = TIMEOUT;
