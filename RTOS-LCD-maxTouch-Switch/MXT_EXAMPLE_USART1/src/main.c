...
int process_touch(t_but botoes[], touchData touch, uint32_t n){
	...
	int y = touch.y;
	for(int i = 0; i < n; i++){
		if(x >= botoes[i].x - botoes[i].width/2 && x <= botoes[i].x + botoes[i].width/2){
			if(y >= botoes[i].y - botoes[i].height/2 && y <= botoes[i].y + botoes[i].height/2){
				return i;
			}
		}
	}
...
void task_lcd(void){
	...
	.x = ILI9488_LCD_WIDTH/2, .y = 50, .status = 0, .callback = &but_callback };
	but0.status = 1;
	...
	t_but but1 = {
		...
		.height = 75,
		.colorOn = COLOR_GREEN,
		.colorOff = COLOR_BLACK,
		.x = ILI9488_LCD_WIDTH/2,
		.y = 140,
		.status = 0,
	...
	while (true) {
		if (xQueueReceive( xQueueTouch, &(touch), ( TickType_t )  500 / portTICK_PERIOD_MS)) {
			...
			// int isTouchXInside = (touch.x > but0.x - but0.width/2) && (touch.x < but0.x + but0.width/2)
			int idx = process_touch(botoes, touch, 5);
			if(idx >= 0) {
				//botoes[idx].status = !botoes[idx].status;
				botoes[idx].callback(&botoes[idx]);
			}
			printf("idx: %d", idx);
			printf("x:%d y:%d\n", touch.x, touch.y);
		}
...
but_callback(t_but *botao) {
	botao->status = !botao->status;
	draw_button_new(*botao);
	printf("Callback \n");
}
